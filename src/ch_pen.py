...
### Transformations/Feature Engineering
    ...
    def feature_dummies(self):
        ...
        df_dow_dum   = df_dow_dum.drop(['dow_0', 'dow_5', 'dow_6'], axis=1)
        ...
        # room ids, floors
        df_rooms_dum = pd.get_dummies(frame['room.id'], prefix='room', drop_first=False)
        df_floor_dum = pd.get_dummies(frame['room.floor'], prefix='floor', drop_first=False)
        ...
        df_roomcap  = pd.cut(frame['room.count_objects'], [-10000, 500, 1000, 25000, 40000],
                             labels=['room_small_cap', 'room_midsize_cap', 'room_large_cap', 'room_xlarge_cap'])
        df_roomcap_dum = pd.get_dummies(df_roomcap, drop_first=False)
        # spot turn-over (some spots change more frequently than others)
        df_spot_freq = pd.cut(frame['spot.count_objects'], [-10000, 500, 1000, 1500, 2500],
                              labels=['spot_constant_freq', 'spot_normal_freq', 'spot_high_freq', 'spot_dynamic_freq'])
        ...
        self.df_dummies   = pd.concat([df_hours_dum, df_dow_dum, df_wom_dum, df_year_dum, df_month_dum, df_quart_dum,
                                       df_rooms_dum, df_floor_dum, df_roomcap_dum, df_spot_freq_dum], axis=1)
        # create single feature matrix
        cols = ['weekend', 'tagged_after_close', 'during_exhibition', 'visitor_drawn', 'meta_store', 'bursty']
...
### Metrics
    def calc_metrics(self, df_objects_in):
        ...
        n_obj_meta_delta        = len( set(self.df_pen.refers_to_object_id.unique()) - set(df_objects_in.id.unique()) )
        n_obs_meta_delta        = self.df_pen[self.df_pen.refers_to_object_id.isin(df_objects_in.id.unique())].shape[0]
        ...
        self.cp_metrics.update(self.metrics)
